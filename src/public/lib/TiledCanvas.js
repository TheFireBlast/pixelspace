...
TiledCanvas.prototype.redrawOnce = function redrawOnce() {
    if (!this._redrawTimeout) this._redrawTimeout = requestAnimationFrame(this.redraw.bind(this, false));
};
...
TiledCanvas.prototype.breakDraw = function breakDraw(chunkX, chunkY, startChunkY, endChunkX, endChunkY) {
    ...
    if (this.breakDrawing && (this.leftTopX !== this.breakDrawingLeftTopX || this.leftTopY !== this.breakDrawingLeftTopY || this.zoom !== this.breakDrawingZoom)) {
        this.breakDrawing = false;
        cancelAnimationFrame(this.breakDrawingRequest);
        delete this.breakDrawingRequest;
        this.redrawOnce();
...
TiledCanvas.prototype.drawChunk = function drawChunk(chunkX, chunkY) {
    if (this.chunks[chunkX] && this.chunks[chunkX][chunkY]) {
        ...
        this.ctx.drawImage(
            ...
            (chunkX * this.settings.chunkSize - this.leftTopX) * this.zoom,
            (chunkY * this.settings.chunkSize - this.leftTopY) * this.zoom,
            this.settings.chunkSize * this.zoom,
            this.settings.chunkSize * this.zoom
        );

        if (!this.breakDrawing && this.chunks[chunkX][chunkY].addedTime) this.drawFade(chunkX, chunkY);
...
TiledCanvas.prototype.drawFade = function drawFade(chunkX, chunkY) {
    ...
    if (deltaAdded > this.settings.fadeTime) delete this.chunks[chunkX][chunkY].addedTime;
    ...
    for (var chunkX = startChunkX; chunkX < endChunkX; chunkX++) {
        for (var chunkY = startChunkY; chunkY < endChunkY; chunkY++) {
            if (this.chunks[chunkX] && this.chunks[chunkX][chunkY] && this.chunks[chunkX][chunkY] !== 'empty') {
                ctx.drawImage(this.chunks[chunkX][chunkY].canvas, chunkX * this.settings.chunkSize - minX, chunkY * this.settings.chunkSize - minY);
            }
        }
    }
};
...
TiledCanvas.prototype.beforeUnloadChunk = function beforeUnloadChunk() {
    return true;
};
...
TiledCanvas.prototype.chunkCount = function chunkCount() {
    ...
    for (var x in this.chunks) for (var y in this.chunks[x]) if (this.chunks[x][y] != 'empty' && this.chunks[x][y]) count++;
...
TiledCanvas.prototype.canBeUnloaded = function canBeUnloaded(cx, cy) {
    return this.chunks[cx] && this.chunks[cx][cy] && Date.now() - (this.chunks[cx][cy].lastDrawn || 0) > this.MIN_INACTIVE_UNLOAD_TIME && !this.chunks[cx][cy].hasBeenDrawnOn && !this.isInView(cx, cy);
};
